...
/**
 * Stores all of the tiles in a single level of a dungeon. Also contains the
 * methods for randomly generating a dungeon.
 * 
...
public class DungeonLevel {
	 ...
	 * 
	 * @param depth
	 *            Affects difficulty of spawns on the level
	 */
	...
	public void generateLevel() {
		for (int i = 0; i < map.length; i++) {
			for (int j = 0; j < map[0].length; j++) {
				map[i][j] = new Tile(true, this, i, j); // runs through each
														// tile and initializes
														// every tile as a
														// "rock" tile
			}
		}
	 ...
	 * with random treasure 2. health potion 3. random amount of gold default: a
	 * wispy spirit monster
	 */
	...
	/*
	 * goes through random tiles and checks if 1.the tile is empty 2.the tile is
	 * in the same region as int region 3.the tile is in a room
	 * 
	 * and returns
	 */
	 ...
	 * 
	 * @param attemptLimit
	 *            The number of times the generator will attempt to place rooms.
	 */
	...
	/**
	 * Prints an "image" of the dungeon level to console. Used to debug dungeon
	 * generator.
	 */
	 ...
	 * 
	 * @param x
	 *            The horizontal position of the room center
	 * @param y
	 *            The vertical position of the room center
	 * @param xLen
	 *            The horizontal width of the room
	 * @param yLen
	 *            The vertical length of the room
	 * @return
	 */
	...
	/**
	 * Generates a random number for an ordinate of the room of a certain
	 * dimension.
	 * 
	 * @param len
	 ...
	 * 
	 * @param x
	 *            the starting horizontal ordinate
	 * @param y
	 *            the starting vertical ordinate
	 */
	...
	private void fill(int x, int y, int[] dir, int t) {
		...
		while (validSpots.size() > 0) {
			...
			if (!validSpots.get(chosenOne).equals(dir)) {
				if (t > 2) {
					fill(validSpots.get(chosenOne)[0] + x, validSpots.get(chosenOne)[1] + y, validSpots.get(chosenOne),
							0);
				} else {
	...
	/**
	 * Returns a list of coordinates which are valid places for the maze to
	 * continue
	 * 
	 * @param x
	 *            horizontal middle ordinate
	 * @param y
	 *            vertical middle ordinate
	...
	private ArrayList<int[]> getValidSpotsOverOne(int x, int y) {
		...
		for (int i = 0; i < 4; i++) {
			if (spots[i][0] + x < 1 || spots[i][0] + x > map.length - 2 || spots[i][1] + y < 1
					|| spots[i][1] + y > map.length - 2 || !map[spots[i][0] + x][spots[i][1] + y].getIsRock()) {
				spots[i] = null;
			}
		}
	...
	/**
	 * Creates all doors necessary to connect all regions of the maze, plus a
	 * random number of extra doors to make the dungeon more interesting.
	 */
	...
	private void createRandomConnectors(int tries) {
		...
		for (int i = 0; i < tries; i++) {
			...
			doors = 0;
			for (int j = 0; j < getValidSpots(connectors.get(choice).getX(), connectors.get(choice).getY())
					.size(); j++) {
	...
	/**
	 * Finds all tiles which are valid doors. A valid door is connected by two
	 * empty spaces of different regions, and is not adjacent to another door.
	 * 
	 ...
	 * 
	 * @param x
	 *            horizontal ordinate of point
	 * @param y
	 *            vertical ordinate of point
	 * @return List of empty spots
	 */
	private ArrayList<int[]> getValidSpots(int x, int y) {
		int[][] spots = { { x - 1, y }, { x + 1, y }, { x, y - 1 }, { x, y + 1 } };
		for (int i = 0; i < 4; i++) {
			if (spots[i][0] < 1 || spots[i][0] > map.length - 2 || spots[i][1] < 1 || spots[i][1] > map.length - 2
					|| map[spots[i][0]][spots[i][1]].getIsRock()) {
				spots[i] = null;
			}
		}
		ArrayList<int[]> validSpots = new ArrayList<int[]>();
	...
	private void addTrap(Tile tile) {
		for (int[] t : getValidSpots(tile.getX(), tile.getY())) {
			if (map[t[0]][t[1]].getRegion() <= rooms.size() && map[t[0]][t[1]].getTopEntity() == null
					&& map[t[0]][t[1]].getRegion() > 0 && !areRegionsConnected(map[t[0]][t[1]].getRegion(), 0)
